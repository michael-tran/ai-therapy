...
* =============================================================================
*/
let vd=null;function yd(e,t,n){if(e===`int32`)return new fe(`int32`,Int32Array.from(t),n);if(e===`int64`){let e=t.map(e=>BigInt(e));return new fe(`int64`,BigInt64Array.from(e),n)}return new fe(`float32`,Float32Array.from(t),n)}self.addEventListener(`message`,async e=>{let t=e.data;try{if(t.type===`init`){let e=t.modelPath||`/model_int8.onnx`;vd=await Se.create(e,{executionProviders:[`webgl`,`wasm`],graphOptimizationLevel:`all`}),self.postMessage({type:`inited`,inputNames:vd.inputNames,outputNames:vd.outputNames,inputMetadata:vd.inputMetadata});return}if(t.type===`generate`){if(!vd)throw Error(`session not initialized`);let e=t.inputIds,n=t.maxLength??50,r=t.eosTokenId??null,i=[...e];for(let e=0;e<n;e++){let e=i.length,t={};for(let n of vd.inputNames){let r=vd.inputMetadata?.[n]||{},a=r.type??`float32`;if(n===`input_ids`)t[n]=yd(a===`int32`?`int32`:`int64`,i,[1,e]);else if(n===`attention_mask`)t[n]=yd(a===`int32`?`int32`:`int64`,Array(e).fill(1),[1,e]);else if(n===`position_ids`)t[n]=yd(a===`int32`?`int32`:`int64`,i.map((e,t)=>t),[1,e]);else if(n.toLowerCase().includes(`past`)||n.toLowerCase().includes(`present`)||n.toLowerCase().includes(`key`)){let e=(Array.isArray(r.dims)&&r.dims.length>=4?r.dims:[1,3,-1,64]).map((e,t)=>e>0?e:e===-1||t===2?0:1),i=e.reduce((e,t)=>e*t,1),a=i>0?Array(i).fill(0):[];t[n]=yd(r.type===`int32`?`int32`:r.type===`int64`?`int64`:`float32`,a,e)}else{let e=(Array.isArray(r.dims)?r.dims:[1]).map(e=>e>0?e:e===-1?0:1),i=e.reduce((e,t)=>e*t,1),a=i>0?Array(i).fill(0):[];t[n]=yd(r.type===`int32`?`int32`:r.type===`int64`?`int64`:`float32`,a,e)}}let n=(await vd.run(t)).logits;if(!n)throw Error(`no logits output from model`);let a=n.dims[2],o=n.data,s=(n.dims[1]-1)*a,c=0,l=o[s];for(let e=1;e<a;e++){let t=o[s+e];t>l&&(l=t,c=e)}let u=c;if(self.postMessage({type:`token`,tokenId:u}),r!==null&&u===r)break;i.push(u)}self.postMessage({type:`done`})}}catch(e){self.postMessage({type:`error`,message:String(e?.message??e)})}})})();